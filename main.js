/* main.js
   Adição ÚNICA:
   - Durante a calibração (isCalibrating === true), pixels pretos (R,G,B < 15)
     são convertidos para vermelho puro.
*/
...
function processFrame() {
    ...
    if (rC) {
        lastRcentroid = { x: rX / rC, y: rY / rC };
        ctx.fillStyle = "red";
        ctx.beginPath();
        ctx.arc(lastRcentroid.x, lastRcentroid.y, 6, 0, Math.PI * 2);
        ctx.fill();
    } else {
        lastRcentroid = null;
    }
