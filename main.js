/* main.js
   ...
   - Mostrar botão/input de upload durante calibração (já presente anteriormente).
   - Após upload do .glb: carregar no <model-viewer>, exibir o modelo, definir camera-target no ponto preto (50mm 50mm 0mm)
     e aplicar orientação baseada nos valores atuais de roll/pitch/yaw (atributo orientation). Chamo updateFraming()
     para que o viewer reconsidere o enquadramento. Nenhuma outra função extra foi adicionada.
*/
...
const modelViewer = document.getElementById("modelViewer"); // <model-viewer> element
...
let currentGlbUrl = null; // para revogar quando trocar
...
if (uploadBtn && glbInput) {
    ...
    glbInput.addEventListener("change", (ev) => {
        ...
        if (f) {
            ...
            // --- AQUI: carregar GLB no model-viewer e posicionar/alinhar ---
            try {
                ...
                currentGlbUrl = URL.createObjectURL(f);
                if (modelViewer) {
                    modelViewer.src = currentGlbUrl;
                    // camera target = ponto preto (âncora) em mm
                    // conforme solicitado: +X=50mm, +Y=50mm, +Z=0
                    modelViewer.setAttribute('camera-target', '50mm 50mm 0mm');

                    // read current device orientation values (texto mostrado na UI)
                    ...
                    // model-viewer orientation uses: roll pitch yaw (degrees)
                    // apply the device values so the model aligns with the computed axes (best-effort)
                    modelViewer.setAttribute('orientation', `${roll}deg ${pitch}deg ${yaw}deg`);

                    // reveal and display the viewer
                    modelViewer.style.display = '';
                    // after changing orientation/camera-target, request framing update if available
                    if (typeof modelViewer.updateFraming === 'function') {
                        try { modelViewer.updateFraming(); } catch (e) { /* ignore */ }
                    }
                }
            } catch (err) {
                console.error("Erro ao carregar GLB:", err);
            }
            // Nota: não há processamento adicional do GLB (nenhuma outra função extra).
        } else {
            glbFile = null;
            if (glbName) glbName.textContent = "";
        }
